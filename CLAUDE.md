...
- **Helpers de PDF compartidos** (`src/utils/pdfHelpers.js`): `seccionTitulo`, `lineaDato`, `encabezadoInstitucional`, `piePaginas`. Extraídos de `casos.js` para que cualquier PDF nuevo (Comité de Convivencia, Plan de Gestión, Oficio de Derivación, Sumarios) tenga la misma tipografía/colores sin copiar y pegar el dibujo del encabezado/pie de página. Si necesitas un PDF nuevo, parte de estos helpers en vez de reinventar el encabezado.
- **`tsQueryBusqueda`** vive en `src/utils/search.js` (no en `casos.js`) — ver gotcha #5 en sección 7.
...
- **Sumarios a funcionarios (Ley Karin) — confidencialidad en dos capas**: un sumario es, técnicamente, un `caso` más con `ambito = 'Funcionario'` y `categoria = CATEGORIA_SUMARIO` (constante reservada, nunca incluida en `CATEGORIAS`). La ruta dedicada `routes/sumarios.js` exige `requireRol("admin","superadmin")` en **todas** sus rutas y delega en las funciones genéricas de `casosController` (`bitacora`, `pasos-protocolo`, `pdf`) reutilizándolas tal cual. Pero esas funciones genéricas no saben de `ambito` — por eso la confidencialidad real está en `casosController.getCasoDetalle` (trata `ambito==='Funcionario'` como 404 para no-admin) y, sobre todo, en `routes/casos.js` vía `router.param("id", ...)`: **cualquier** ruta de `/casos` que incluya `:id` (incluidas las de adjuntos/derivaciones/mediaciones, que no pasan por `getCasoDetalle`) queda bloqueada para un sumario si quien pide no es admin/superadmin. Si se agrega una ruta nueva con `:id` bajo `/casos`, este guard la cubre automáticamente — no hay que repetir el chequeo a mano, pero sí hay que recordar que existe antes de "simplificarlo".
...
- `casos` — expediente: estudiante (el "principal"), `categoria` (**`VARCHAR(60)` desde 2026-06-27, ya no enum** — ver nota más abajo), `estado_caso` (enum: Abierto/En seguimiento/Cerrado), `ambito` (enum `ambito_caso`: `'Estudiantil'`/`'Funcionario'`, default `'Estudiantil'`), responsable, flags PIE/NEE, JUNAEB, curso, `estudiante_id` (FK opcional a `estudiantes`), etc.
...
- `derivaciones` — derivaciones externas (Carabineros, Fiscalía, OLN — Oficina Local de la Niñez, ex-OPD —, GES / Programa de Salud Mental Escolar, etc.), con `estado_derivacion` enum, flag de denuncia obligatoria y `confidencial BOOLEAN` (oculta `notas`/`folio_externo` a quien no sea admin/superadmin/Psicólogo PIE — mismo criterio que `diagnostico_pie`, ver `puedeVerConfidencial` en `casos.js` y `derivaciones.js`). `GET /casos/:id/derivaciones/:derId/pdf` genera un "Oficio de Derivación" interno (plantilla prellenada, no el formulario oficial de la institución externa).
...
- `actas_comite_convivencia` + `acta_comite_compromisos` — actas del Comité de Buena Convivencia Escolar / Consejo Escolar (a nivel de colegio, no de un caso), mismo patrón que `mediaciones`/`mediacion_compromisos`. Cada acta se puede imprimir en PDF; el endpoint `GET /convivencia/plan-gestion.pdf?anio=YYYY` junta estas actas + actividades de convivencia + protocolos vigentes + estadísticas de casos del año en un solo "Plan de Gestión Anual".
- `estudiantes` — catálogo **opcional** por colegio (nombre, curso, rut, activo) que solo alimenta autocompletado (`GET /estudiantes?search=`); `casos.estudiante`/`caso_estudiantes_adicionales.nombre` siguen siendo texto libre siempre — el vínculo vía `estudiante_id` es aditivo y nunca obligatorio.
- `session` — tabla de sesiones de `connect-pg-simple` (no tocar manualmente).

> **`categoria_caso` (enum) → `VARCHAR(60)` (2026-06-27).** `casos.categoria`, `protocolos.categoria` y `protocolos_colegio.categoria` eran un ENUM nativo de Postgres. Agregar un valor a un ENUM existente (`ALTER TYPE ... ADD VALUE`) no se puede usar en la misma transacción en la que también se inserta una fila que use ese valor nuevo, y el patrón de migraciones de este proyecto re-ejecuta `schema.sql` completo en una sola transacción — así que agregar una categoría nueva (p. ej. "Aula Segura") siempre habría chocado con esa restricción. Se convirtieron esas 3 columnas a `VARCHAR(60)`: la validación real de qué valores son válidos ya vive en Zod (`CATEGORIAS` en `validation/constants.js`), el ENUM era solo defensa en profundidad redundante. El tipo `categoria_caso` quedó sin usar en el esquema (no se eliminó, es inofensivo). Por la misma razón, `v_casos` se redefine dos veces en `schema.sql`: la original (sin `ambito`/`estudiante_id`) y una segunda `CREATE OR REPLACE VIEW` al final del archivo, después de que esas columnas existen — la vista debe eliminarse (`DROP VIEW`) antes de poder alterar el tipo de `casos.categoria`, porque Postgres no permite cambiar el tipo de una columna de la que depende una vista.
...
- **Módulos JS** (`public/js/*.js`): cada uno es un IIFE que expone un objeto global (`App`, `Auth`, `Casos`, `Bitacora`, `Mediaciones`, `Capacitaciones`, `Convivencia`, `ComiteConvivencia`, `Equipo`, `Colegios`, `Config`, `Dashboard`, `DashboardSostenedor`, `Estudiantes`, `Sumarios`, `Api`, `Offline`). Los botones del HTML llaman directo a estas funciones vía `onclick="Modulo.funcion()"` — no hay event delegation centralizado. Cada módulo nuevo debe agregarse también a `FRONTEND_MODULE_GLOBALS` en `eslint.config.js`, o ESLint lo marca `no-undef`/`no-unused-vars` en cuanto otro módulo lo referencia.
...
- **Vistas actuales**: `view-central` (superadmin: colegios/sostenedores, con botón "Ver Dashboard" por sostenedor), `view-dashboard`, `view-casos`, `view-detalle` (expediente), `view-equipo` (incluye el catálogo opcional de estudiantes), `view-convivencia` (actividades/medidas/protocolos personalizados + Comité de Convivencia + Plan de Gestión Anual), `view-config` (umbrales, retención/purga, metas PME, auditoría), `view-dashboard-sostenedor` (satélite sin ítem de nav propio, se abre desde `view-central`), `view-sumarios` + `view-detalle-sumario` (ítem de nav "Sumarios", oculto salvo admin/superadmin con colegio en contexto — confidencialidad real en el backend, ver sección 4).
- **Modo rápido de apertura de caso**: `modalApertura` abre por defecto mostrando solo Estudiante/Categoría/Responsable/Descripción; curso, JUNAEB, NEE/PIE y estudiantes adicionales quedan colapsados detrás de "+ Mostrar más campos" (`Casos.toggleCamposAvanzadosApertura`). Mismo endpoint `POST /casos` de siempre — es un cambio puramente de presentación, para poder registrar un incidente desde el celular en segundos.
...
5. **`to_tsquery('spanish', ...)` no acepta texto plano multi-palabra.** Pasarle directamente `inmutable_unaccent($n) || ':*'` revienta con "error de sintaxis en tsquery" en cuanto el término de búsqueda tiene más de una palabra (ej. "Juan Pérez"). Usar el helper `tsQueryBusqueda(param)` en `src/utils/search.js`, que convierte el texto en sintaxis válida (`regexp_replace(..., '\s+', ':* & ', 'g') || ':*'`) antes de pasarlo a `to_tsquery`. Lo usan `casos.js` (estudiantes) y `estudiantes.js` (catálogo/autocompletado).
...
7. **No se puede `ALTER COLUMN ... TYPE` sobre una columna de la que depende una vista** (Postgres tira "no se puede alterar el tipo de una columna usada en una regla o vista"). Si necesitas cambiar el tipo de `casos.categoria` (o cualquier columna que `v_casos` exponga), hay que `DROP VIEW v_casos` antes del `ALTER TABLE` y volver a crearla después con `CREATE OR REPLACE VIEW` (ver el comentario en `schema.sql` junto a `DROP VIEW IF EXISTS v_casos`). Mismo cuidado aplica a cualquier vista futura.
8. **Reusar el motor de `casos` para un dominio confidencial (sumarios Ley Karin) no protege automáticamente las rutas de sub-recursos genéricas.** `casosController.getCasoDetalle` bloquea la lectura del expediente completo, pero los endpoints de adjuntos/derivaciones/mediaciones bajo `/casos/:id/...` no pasan por ahí — sin el guard `router.param("id", ...)` en `routes/casos.js`, cualquier usuario con sesión podía leer/escribir adjuntos de un sumario adivinando el id. Si agregas un dominio confidencial nuevo que reutilice `casos`, verifica que el guard cubra también sus sub-rutas, no solo el endpoint principal.
